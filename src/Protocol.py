...
class Protocol():
    ...
    def get_message(colour, timeout_ns=30 * 10**9, verbose=False):
        ...
        try:
            ...
            if verbose:
                print(f"{Protocol.sockets[colour]['name']} timed out. " +
                      "Nothing received.")
            return ("NO MESSAGE", -1)
            ...
            if verbose:
                print(f"{Protocol.sockets[colour]['name']} socket " +
                      "ended unexpectedly.")
            return ("NO MESSAGE", -1)

        if verbose:
            print(f"Received {data.decode('utf-8').strip()} from " +
    ...
    def send_message(colour, message, verbose=False):
        ...
        try:
            ...
            if verbose:
                print(f"Failed to send {message.strip()} to " +
