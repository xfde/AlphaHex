...
def main():
    ...
    for argument in argv:
        ...
        if ("board_size=" in argument or "b=" in argument):
            try:
                ...
                print("ERROR: Board size argument is not in valid",
                      "format. Aborted.")
                return
    ...
    if (len(agents) > 2):
        ...
        if (len(agents) == 1 and double):
            print(
                "NOTICE: Double agent enabled. Your agent will play both sides."
            )
            try:
                agent1 = {
            ...
            print("NOTICE: Fewer than two agents specified.",
