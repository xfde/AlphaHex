...
class Game():
    ...
    def _play(self):
        ...
        * Timeout - one of the agents fails to send a message before
        ...
        self._start_protocol(self._players[Colour.RED]['run string'],
        ...
        # start the game
        self._send_message(verbose_message=("Started game of Hex. Board is " +
        ...
        while (not self._board.has_ended()):
            ...
            # recorded.
            self._send_message(verbose_message=self._board.print_board(
                bnf=False))
    ...
    def _send_message(self,
        ...
        if (protocol_message != ""):
            if (start):
                Protocol.send_message(Colour.RED,
                                      f"{protocol_message}R\n",
                                      verbose=self._print_protocol)
                Protocol.send_message(Colour.BLUE, f"{protocol_message}B\n")
            else:
                Protocol.send_message(Colour.RED,
                                      protocol_message,
                                      verbose=self._print_protocol)
                Protocol.send_message(Colour.BLUE, protocol_message)
    ...
    def _get_move(self):
        ...
        answer, move_time = Protocol.get_message(self._player, time_left,
    ...
    def _end_game(self, status):
        ...
        self._send_message(verbose_message=self._board.print_board(bnf=False))

        # calculate total time elapsed
        total_time = time() - self._start_time

        # calculate mean move times
        means = {'Total': 0, Colour.RED: 0, Colour.BLUE: 0}
        ...
        for colour in Colour:
            if (self._players[colour]['turns'] > 0):
                means[colour] = int(self._players[colour]['time'] /
        ...
        if (status == EndState.WIN):
            ...
            log_message = (f"0,{self._players[self._player]['name']}," +
        ...
        log_message += (f"0,Total,{self._turn},{total_time},{means['Total']}\n")
        ...
        Protocol.close(kill_children=self._kill_bots,
                       verbose=self._print_protocol)
    ...
    def _start_protocol(self, s1, name1, s2, name2):
        ...
        self._has_connected = Protocol.accept_connection(
            s1, name1, Game.MAXIMUM_TIME, self._silent_bots,
            self._print_protocol)
        if (not self._has_connected):
        ...
        self._has_connected = Protocol.accept_connection(
            s2, name2, Game.MAXIMUM_TIME, self._silent_bots,
            self._print_protocol)
        if (not self._has_connected):
    ...
    def _start_log(self):
        ...
        with open(self._log_path, "w") as f:
            ...
            f.write(f"Board is {self._board.get_size()}x" +
                    f"{self._board.get_size()}.\n")
